	...
	return new Proxy(target, {
		...
		set (target, prop, value) {
			if (Array.isArray(value)) {
				target[prop] = ArrayProxy(target, prop, value, typeclass.type[0])
			} else if (isRawObject(value)) {
				target[prop] = ObjectProxy(target, prop, value, typeclass.type[0])
			} else {
				target[prop] = typeclass.type[0].coerce(value)
			}
			// this will cause _updates to bubble up until they hit the instance
			target._updates[prop] = typeclass.type[0].coerce(value)
			return true
		},
	})
}
...
function ObjectProxy (parent, field, target, typeclass) {
	...
	return new Proxy(target, {
		set (target, prop, value) {
			if (Array.isArray(value)) {
				target[prop] = ArrayProxy(target, prop, value, typeclass.type[prop])
			} else if (isRawObject(value)) {
				target[prop] = ObjectProxy(target, prop, value, typeclass.type[prop])
			} else {
				target[prop] = typeclass.type[prop].coerce(value)
			}
			// this will cause _updates to bubble up until they hit the instance
			target._updates[prop] = typeclass.type[prop].coerce(value)
			return true
		},
	})
}
