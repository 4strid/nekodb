...
prototype._isRawObject = function (obj) {
	if (obj instanceof Date || obj === null || Array.isArray(obj) || obj instanceof this.client.ObjectID) {
		return false
	}
...
function promisifyHook (hook, instance) {
	...
	return returnedPromise instanceof Promise ? returnedPromise : promisifed
}
...
prototype._save = function (instance) {
	...
	return promisifyHook(this.prevalidate, instance).then(() => {
		...
		if (this.hasOwnProperty('presave')) {
			Object.assign(oldValues, instance)
		}
		return promisifyHook(this.presave, instance)
	}).then(() => {
		return this.client.save(this.name, instance)
	}).then(saved => {
		Object.assign(instance, oldValues)
...
prototype._saveRefs = function (instance) {
	...
	for (const field in this.references) {
		...
		const refInstance = this.references[field].process(instance[field], reference => {
			...
			if (this._isRawObject(reference)) {
				didSave = true
