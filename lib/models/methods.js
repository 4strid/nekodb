...
prototype.countDocuments = function (query) {
	this.coerceQuery(query)
	return this.client.countDocuments(this.name, query)
}

prototype.estimatedDocumentCount = function (query) {
	this.coerceQuery(query)
	return this.client.estimatedDocumentCount(this.name, query)
}
	...
	for (const field in this.references) {
		...
		if (didSave) {
			...
			if (Array.isArray(refInstance)) {
				toSave.push(Promise.all(refInstance.map(ref => {
					// only try to save if it's actually an instance
				...
				// only try to save if it's actually an instance
				toSave.push(refInstance instanceof Instance ? refInstance.save().catch(err => {
					err.index = i
					throw err
				}) : Promise.resolve(refInstance))
			}
		}
	}
