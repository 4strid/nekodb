...
Typeclass.prototype.coerce = function (value) {
	...
	// for option types, coerce to the first type in the option with special casing for null
	if (type.isOption(this.type)) {
		// if there exists a typeclass whose type is null
		if (this.type.filter(typeclass => typeclass.type === null).length) {
